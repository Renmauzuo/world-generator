(function () {
     ...
     */
    function collectAncestorTags(node, typeMap) {
        const tags = new Set();
        let current = node;
        while (current) {
            const template = typeMap[current.type];
            if (template === null || template === void 0 ? void 0 : template.tags) {
    ...
    function getRegisteredNodes(...types) {
        const result = [];
     ...
     */
    function registerTree(node, registeredTypes) {
    ...
    function damageString(damageDice, damageDieSize, damageBonus = 0) {
        ...
        if (maxDamage <= 1)
            return '1';
        if (damageDieSize === 1)
            return String(damageDice + damageBonus);
    ...
    const monsterList = {
        airElemental: {
            name: 'Air Elemental',
            type: creatureTypes$1.elemental,
        ...
        angel: {
            name: 'Angel',
            type: creatureTypes$1.celestial,
        ...
        kraken: {
            name: 'Kraken',
            type: creatureTypes$1.monstrosity,
            subtype: 'titan',
            alignment: alignments.chaoticEvil,
            lockedStats: {
                armorDescription: armorMaterials.natural,
        ...
        shadow: {
            ...
            variants: {
                shadow: { name: 'Shadow', type: creatureTypes$1.undead, lockedStats: { resistances: [damageTypes.mundanePhysical], vulnerabilities: [damageTypes.radiant] }, stats: { 2: { name: 'Shadow' } } },
                detachedShadow: { name: 'Detached Shadow', type: creatureTypes$1.fey, lockedStats: { resistances: [damageTypes.mundanePhysical], vulnerabilities: [damageTypes.radiant] }, stats: { 2: { name: 'Detached Shadow' } } },
                reflection: { name: 'Reflection', type: creatureTypes$1.fey, lockedStats: { slug: 'reflection', resistances: [damageTypes.mundanePiercingSlashing], vulnerabilities: [damageTypes.bludgeoning] }, stats: { 2: { name: 'Reflection' } } },
            },
            ...
            if (numCR > numTargetCR) {
                if (!benchmarks.upper || benchmarks.upper.cr > numCR) {
                    ...
                    for (const s of statList)
                        benchmarks.upper[s] = statBlock[s];
                }
            }
            else {
                if (!benchmarks.lower || benchmarks.lower.cr < numCR) {
                    ...
                    for (const s of statList)
                        benchmarks.lower[s] = statBlock[s];
                }
            }
        }
        return benchmarks;
            ...
            if (benchmarkLevel !== undefined) {
                ...
                for (const cr in sourceStats) {
                    ...
                    if (numCR <= Number(targetCR)) {
                        ...
                        if (traitData?.spellcastingLevel !== undefined)
                            benchmarkCR = numCR;
                    }
                }
    ...
    function applyLegendary(statblock, count) {
        ...
        const abilityMods = statblock.abilityModifiers;
        ...
        if (statblock.attacks) {
            for (const attackKey in statblock.attacks) {
                ...
                if (!atk.damageDice || !atk.damageDieSize)
                    continue;
    ...
    function scaleMonster(monster, targetCR, options = {}) {
        ...
        if (selectedVariant?.stats) {
            for (const cr in selectedVariant.stats) {
                ...
                sourceStats[numCR] = sourceStats[numCR]
                    ? mergeObjects(sourceStats[numCR], selectedVariant.stats[numCR])
                    : { ...selectedVariant.stats[numCR] };
            }
        }
        let derivedStats = { cr: targetCR, gender: selectedMonster.gender ?? 4 };
        ...
        let currentRace;
        let currentLineage;
        if (type === creatureTypes$1.humanoid) {
        ...
        for (const cr in sourceStats) {
            ...
            if (numCR <= numTargetCR) {
                ...
                if (Array.isArray(crActions)) {
                    for (const actionName of crActions)
        ...
        if (derivedStats.bonusArmor === undefined) {
            ...
            if (acBenchmarks) {
                for (const key of ['upper', 'lower']) {
                    ...
                    if (b)
                        b.ac = 10 + b.bonusArmor + abilityScoreModifier(b.dex);
                }
        ...
        if (!derivedStats.hitDice) {
            ...
            if (hpBenchmarks) {
                ...
                const hpPerHD = hitPointsPerHitDie({ size: derivedStats.size, con: derivedStats.con });
        ...
        if (type === creatureTypes$1.humanoid && selectedMonster.race === raceKeys.any && currentRace && currentRace !== races[0]) {
            const abilityMods = derivedStats.abilityModifiers;
                ...
                for (const cantripKey of allCantrips) {
                    ...
                    if (spell.isAttack && spell.damageDieSize && spell.damageType) {
                        ...
                        if (!derivedStats.castingStat) {
                            if (abilityMods.cha === bestMentalMod)
                                derivedStats.castingStat = 'cha';
                            else if (abilityMods.wis === bestMentalMod)
                                derivedStats.castingStat = 'wis';
                            else
                                derivedStats.castingStat = 'int';
                        }
                    }
                    else if (spell.saveAbility && spell.damageDieSize && spell.damageType) {
                        // Save-based damage cantrip — generate an action entry
                        if (!derivedStats.actions)
                            derivedStats.actions = {};
     ...
     */
    function npcNameGenerator(node) {
        var _a, _b, _c, _d, _e;
        const race = (_b = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.race) !== null && _b !== void 0 ? _b : 'human';
        const gender = (_d = (_c = node.attributes) === null || _c === void 0 ? void 0 : _c.gender) !== null && _d !== void 0 ? _d : '';
        // Non-binary NPCs draw from either name pool randomly
        const isFemale = gender === 'Female' || (gender === 'Non-binary' && rand(1, 2) === 2);
        const generator = (_e = racialGenerators[race]) !== null && _e !== void 0 ? _e : racialGenerators.human;
    ...
    function generateHumanName(isFemale) {
    ...
    function generateDwarfName(isFemale) {
    ...
    function generateElfName(isFemale) {
    ...
    function generateHalflingName(isFemale) {
    ...
    function generateGnomeName(isFemale) {
    ...
    // ---------------------------------------------------------------------------
    function generateHalfElfName(isFemale) {
    ...
    function generateHalfOrcName(isFemale) {
    ...
    function generateDragonbornName(isFemale) {
    ...
    function generateTieflingName(isFemale) {
     ...
     */
    function generateNpcDescription() {
     ...
     */
    function findNeighborRegions(node, typeMap) {
        var _a, _b;
        ...
        let scope;
        cursor = node.parent;
        let highestGeographic;
        while (cursor) {
            const tags = (_a = typeMap[cursor.type]) === null || _a === void 0 ? void 0 : _a.tags;
            if (tags) {
        ...
        while (stack.length > 0) {
            ...
            const tags = (_b = typeMap[current.type]) === null || _b === void 0 ? void 0 : _b.tags;
            if ((tags === null || tags === void 0 ? void 0 : tags.includes('region')) && current.name && !ownAncestors.has(current)) {
     ...
     */
    function tryNeighborRumor(node, typeMap) {
        var _a, _b;
        const neighbors = findNeighborRegions(node, typeMap);
        if (neighbors.length === 0)
            return null;
        const target = randFromArray(neighbors);
        const targetTags = (_b = (_a = typeMap[target.type]) === null || _a === void 0 ? void 0 : _a.tags) !== null && _b !== void 0 ? _b : [];
    ...
    function buildHookProfile(node, typeMap) {
        var _a, _b, _c, _d, _e, _f, _g;
        const template = typeMap[node.type];
        const baseCreature = (_a = template === null || template === void 0 ? void 0 : template.creature) !== null && _a !== void 0 ? _a : '';
        const parentType = (_c = (_b = node.parent) === null || _b === void 0 ? void 0 : _b.type) !== null && _c !== void 0 ? _c : '';
        const alignment = (_e = (_d = node.attributes) === null || _d === void 0 ? void 0 : _d.alignment) !== null && _e !== void 0 ? _e : '';
        const worship = (_g = (_f = node.attributes) === null || _f === void 0 ? void 0 : _f.worship) !== null && _g !== void 0 ? _g : '';
        const tags = collectAncestorTags(node, typeMap);
        let actionableChance = 15; // baseline — most NPCs are rumor-mongers
        const actionablePools = [selfContainedActionable];
        const rumorPools = [genericRumors];
    ...
    function tryWorldAwareHook() {
        ...
        if (roll === 1) {
            ...
            if (crypts.length > 0) {
                ...
                const place = (_a = target.name) !== null && _a !== void 0 ? _a : 'an ancient crypt nearby';
            ...
            if (fortresses.length > 0) {
                ...
                const place = (_b = target.name) !== null && _b !== void 0 ? _b : 'a demonic fortress';
     ...
     */
    function generateAdventureHook(node, typeMap) {
     ...
     */
    let objectTypesRef$3 = {};
    function setObjectTypesRef$3(ref) {
        objectTypesRef$3 = ref;
    }
    /** All valid Challenge Rating values, derived from the averageStats table, sorted numerically. */
    const challengeRatingList = Object.keys(averageStats).map(Number).sort((a, b) => a - b);
    function populationDensityValue(node) {
        var _a, _b;
        if ((_b = (_a = node.parent) === null || _a === void 0 ? void 0 : _a.attributes) === null || _b === void 0 ? void 0 : _b.populationDensity) {
            const inheritedDensityWeight = {};
    ...
    function racialDemographicsValue(node) {
        ...
        if ((_b = (_a = node.parent) === null || _a === void 0 ? void 0 : _a.attributes) === null || _b === void 0 ? void 0 : _b.racialDemographics) {
     ...
     */
    function selectNpcRace(node) {
        var _a;
        // Walk up the parent chain to find racial demographics
        let current = node.parent;
        let demographics;
        while (current) {
            if ((_a = current.attributes) === null || _a === void 0 ? void 0 : _a.racialDemographics) {
        ...
        const allRaces = Object.keys(races$1);
     ...
     */
    function selectDeityDomain(alignment, element) {
        const eligible = Object.entries(deityDomains).filter(([_key, domain]) => {
            if (domain.alignments.length > 0) {
                if (!domain.alignments.some(a => alignment.includes(a)))
                    return false;
            }
            if (domain.excludeAlignments.length > 0) {
                if (domain.excludeAlignments.some(a => alignment.includes(a)))
                    return false;
            }
            if (domain.elements.length > 0) {
                if (!domain.elements.some(e => e === element))
                    return false;
            }
            if (domain.excludeElements.length > 0) {
                if (domain.excludeElements.some(e => e === element))
                    return false;
            }
            return true;
        });
    ...
    const creatureTypeElementAffinity = {
    ...
    const creatureTypeDomainAffinity = {
        beast: ['nature', 'beasts'],
        fey: ['nature', 'trickery'],
        undead: ['death', 'shadow'],
     ...
     */
    function getMaxRealCR(template) {
        let maxCR = 0;
     ...
     */
    function scoreCreatureForDeity(template, variantData, alignment, element, domainKey) {
        var _a, _b, _c, _d, _e, _f, _g;
        let score = 0;
        // 1. Max real CR — check both base stats and variant stats
        let maxCR = getMaxRealCR(template);
        if (variantData === null || variantData === void 0 ? void 0 : variantData.stats) {
        ...
        if (maxCR < 2)
            return 0;
        const creatureType = (_a = template.type) !== null && _a !== void 0 ? _a : '';
        const creatureAlignment = (_b = template.alignment) !== null && _b !== void 0 ? _b : '';
        const planarAlignment = (_c = alignmentToPlanar[creatureAlignment]) !== null && _c !== void 0 ? _c : '';
        // 2. Creature type affinity (0–15)
        const isGood = alignment.includes('Good');
        const isEvil = alignment.includes('Evil');
        if (creatureType === 'celestial' && isGood)
            score += 15;
        else if (creatureType === 'fiend' && isEvil)
            score += 15;
        else if (creatureType === 'undead' && (isEvil || element === 'Negative Energy'))
            score += 12;
        else if (creatureType === 'dragon')
            score += 12;
        else if (creatureType === 'elemental')
            score += 10;
        else if (creatureType === 'beast' || creatureType === 'monstrosity')
            score += 8;
        else if (creatureType === 'fey')
            score += 8;
        else if (creatureType === 'plant')
            score += 5;
        // Penalize mismatches
        else if (creatureType === 'celestial' && isEvil)
            return 0;
        else if (creatureType === 'fiend' && isGood)
            return 0;
        else if (creatureType === 'undead' && isGood)
            return 0;
        ...
        if (planarAlignment && planarAlignment === alignment) {
            ...
            if ((creatureGood && isEvil) || (creatureEvil && isGood))
                return 0;
        }
        else {
        ...
        const immunities = [
            ...((_e = (_d = template.lockedStats) === null || _d === void 0 ? void 0 : _d.immunities) !== null && _e !== void 0 ? _e : []),
            ...((_g = (_f = variantData === null || variantData === void 0 ? void 0 : variantData.lockedStats) === null || _f === void 0 ? void 0 : _f.immunities) !== null && _g !== void 0 ? _g : [])
        ];
     ...
     */
    function selectDeityCreature(alignment, element, domainKey) {
        const candidates = [];
        for (const [id, template] of Object.entries(monsterList)) {
            ...
            if (template.variants) {
                for (const [variantKey, variantData] of Object.entries(template.variants)) {
     ...
     */
    function deitySetup(node, legendary) {
        var _a, _b, _c, _d, _e;
        const alignment = (_b = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.alignment) !== null && _b !== void 0 ? _b : '';
        const element = (_d = (_c = node.attributes) === null || _c === void 0 ? void 0 : _c.element) !== null && _d !== void 0 ? _d : '';
        ...
        node.attributes.variant = (_e = result.variant) !== null && _e !== void 0 ? _e : '';
     ...
     */
    function avatarSetup(node, deities, locationTags) {
        ...
        if (deities.length > 0) {
            ...
            for (let i = 0; i < deities.length; i++) {
                ...
                const deityDomain = (_f = (_e = deities[i].attributes) === null || _e === void 0 ? void 0 : _e.domain) !== null && _f !== void 0 ? _f : '';
            ...
            node.attributes.creature = (_h = (_g = deity.attributes) === null || _g === void 0 ? void 0 : _g.creature) !== null && _h !== void 0 ? _h : 'angel';
            node.attributes.variant = (_k = (_j = deity.attributes) === null || _j === void 0 ? void 0 : _j.variant) !== null && _k !== void 0 ? _k : '';
            node.attributes.domain = (_m = (_l = deity.attributes) === null || _l === void 0 ? void 0 : _l.domain) !== null && _m !== void 0 ? _m : '';
            node.attributes.deityName = (_o = deity.name) !== null && _o !== void 0 ? _o : deity.type;
        }
        else {
            ...
            const result = selectDeityCreature(alignment, element, domainKey);
            node.attributes.creature = result.creature;
            node.attributes.variant = (_p = result.variant) !== null && _p !== void 0 ? _p : '';
            const domainData = deityDomains[domainKey];
    ...
    function domainMatchesTags(domainName, tags) {
        for (const tag of tags) {
     ...
     */
    function selectDomainByTags(alignment, element, tags) {
        // Collect all domains that match any location tag
        const tagMatchedDomains = new Set();
        for (const tag of tags) {
        ...
        if (tagMatchedDomains.size > 0) {
            ...
            const eligible = Object.entries(deityDomains).filter(([_key, domain]) => {
                if (!tagMatchedDomains.has(domain.name))
                    return false;
                if (domain.alignments.length > 0 && !domain.alignments.some(a => alignment.includes(a)))
                    return false;
                if (domain.excludeAlignments.length > 0 && domain.excludeAlignments.some(a => alignment.includes(a)))
                    return false;
                if (domain.elements.length > 0 && !domain.elements.some(e => e === element))
                    return false;
                if (domain.excludeElements.length > 0 && domain.excludeElements.some(e => e === element))
                    return false;
                return true;
            });
            if (eligible.length > 0) {
     ...
     */
    function selectNpcAlignment(worship, race, deities) {
        var _a;
        // Check if worship matches a deity name — use that deity's alignment
        if (deities.length > 0) {
            const matchedDeity = deities.find(d => { var _a; return ((_a = d.name) !== null && _a !== void 0 ? _a : d.type) === worship; });
            if ((_a = matchedDeity === null || matchedDeity === void 0 ? void 0 : matchedDeity.attributes) === null || _a === void 0 ? void 0 : _a.alignment) {
        ...
        if (domainEntry) {
            ...
            const compatible = alignmentList.filter(a => {
                if (domain.alignments.length > 0 && !domain.alignments.some(ax => a.includes(ax)))
                    return false;
                if (domain.excludeAlignments.length > 0 && domain.excludeAlignments.some(ax => a.includes(ax)))
                    return false;
                return true;
            });
        ...
        const raceData = races$1[race];
        if (raceData === null || raceData === void 0 ? void 0 : raceData.alignmentBias) {
    ...
    function npcSetup(node) {
        ...
        node.attributes.adventureHook = generateAdventureHook(node, objectTypesRef$3);
    }
    /**
     ...
     */
    function dragonbornNpcSetup(node) {
        var _a, _b;
        // Map dragon color to lineage name (capitalize first letter)
        const color = (_b = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.dragonColor) !== null && _b !== void 0 ? _b : '';
        if (color) {
     ...
     */
    function selectNpcLineage(raceKey) {
        var _a;
        const raceEntry = races.find(r => r.name === raceKey);
        if (!((_a = raceEntry === null || raceEntry === void 0 ? void 0 : raceEntry.lineages) === null || _a === void 0 ? void 0 : _a.length))
            return '';
     ...
     */
    function selectWorship(deities) {
        var _a;
        ...
        if (deities.length > 0 && rand(1, 10) <= 7) {
            ...
            const selectedIndex = parseInt(weightedRand(weights), 10);
            const deity = deities[selectedIndex];
            return (_a = deity.name) !== null && _a !== void 0 ? _a : deity.type;
        }
     ...
     * @param deities - Array of existing deity nodes from the registry
     */
    function templeSetup(node, deities) {
        node.attributes.worship = selectWorship(deities);
    }
     ...
     */
    function settlementSetup(node, objectTypes) {
     ...
     */
    function environmentalResistanceSetup(node) {
        var _a;
        const temp = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.temperature;
        if (temp === 'Cold') {
     ...
     */
    function speciesSetup(node, objectTypes) {
     ...
     */
    function getGroupCreatureType(node, objectTypes) {
        var _a, _b;
        return (_b = (_a = objectTypes[node.type]) === null || _a === void 0 ? void 0 : _a.speciationId) !== null && _b !== void 0 ? _b : null;
    }
    /**
     ...
     */
    function findExistingSpecies(node, creatureType, objectTypes) {
        var _a;
        const result = { regionSpecies: [], continentSpecies: [] };
        let current = node.parent;
        while (current) {
            const template = objectTypes[current.type];
            const typeTags = template === null || template === void 0 ? void 0 : template.tags;
            const registry = (_a = current.attributes) === null || _a === void 0 ? void 0 : _a.speciesRegistry;
            if (registry) {
                const matching = registry.filter(s => s.creature === creatureType);
                if (matching.length > 0) {
                    if (typeTags === null || typeTags === void 0 ? void 0 : typeTags.includes('region')) {
                        result.regionSpecies.push(...matching);
                    }
                    else if (typeTags === null || typeTags === void 0 ? void 0 : typeTags.includes('continent')) {
     ...
     */
    function registerSpeciesOnAncestor(node, creatureType, speciesName, objectTypes) {
        ...
        while (current) {
            const template = objectTypes[current.type];
            const typeTags = template === null || template === void 0 ? void 0 : template.tags;
            if (typeTags === null || typeTags === void 0 ? void 0 : typeTags.includes(targetTag)) {
                // Found the target scope — store here
                if (!current.attributes)
                    current.attributes = {};
                if (!current.attributes.speciesRegistry)
                    current.attributes.speciesRegistry = [];
            ...
            if (typeTags === null || typeTags === void 0 ? void 0 : typeTags.includes('continent')) {
                fallback = current;
            }
            current = current.parent;
        }
        ...
        if (fallback) {
            if (!fallback.attributes)
                fallback.attributes = {};
            if (!fallback.attributes.speciesRegistry)
                fallback.attributes.speciesRegistry = [];
     ...
     */
    function generateSpeciesName(context) {
     ...
     */
    function getGeographicContext(node, objectTypes) {
        var _a;
        const context = {
            continentName: null,
            regionName: null,
        ...
        while (current) {
            ...
            if (!context.temperature && ((_a = current.attributes) === null || _a === void 0 ? void 0 : _a.temperature)) {
                context.temperature = current.attributes.temperature;
            }
            // Find the nearest named region (types tagged 'region')
            if (!context.regionName && (typeTags === null || typeTags === void 0 ? void 0 : typeTags.includes('region')) && current.name) {
            ...
            if (!context.continentName && (typeTags === null || typeTags === void 0 ? void 0 : typeTags.includes('continent')) && current.name) {
     ...
     */
    function getBiomeNounFromTags(tags) {
     ...
     */
    function deriveGeographicAdjective(placeName) {
        // Strip "The " prefix
     ...
     */
    function generateDescriptiveSpecies(context) {
        const pools = [];
    ...
    function generateTitle(options = {}) {
        ...
        const isGood = (_a = alignment === null || alignment === void 0 ? void 0 : alignment.includes('Good')) !== null && _a !== void 0 ? _a : false;
        const isEvil = (_b = alignment === null || alignment === void 0 ? void 0 : alignment.includes('Evil')) !== null && _b !== void 0 ? _b : false;
        const isLawful = (_c = alignment === null || alignment === void 0 ? void 0 : alignment.includes('Lawful')) !== null && _c !== void 0 ? _c : false;
        const isChaotic = (_d = alignment === null || alignment === void 0 ? void 0 : alignment.includes('Chaotic')) !== null && _d !== void 0 ? _d : false;
        // ─── Layer 1: Generic titles (always available) ───
        pool.push('the Ancient', 'the Undying', 'the Veiled', 'the Silent', 'the Awakened', 'the Unbroken', 'the Shrouded', 'the Dreaming', 'the Forgotten', 'the Nameless');
        // ─── Layer 2: Power tier (CR-gated) ───
        if (cr !== undefined) {
            if (cr < 5) {
                pool.push('the Humble', 'the Watchful', 'the Patient', 'the Quiet', 'the Wandering', 'the Unseen', 'the Drifting', 'the Minor');
            }
            if (cr >= 5) {
                pool.push('the Mighty', 'the Feared', 'the Formidable', 'the Relentless', 'the Resolute', 'the Stalwart', 'the Unyielding', 'the Bold');
            }
            if (cr >= 12) {
                pool.push('the Terrible', 'the Magnificent', 'the Dread', 'the Legendary', 'the Indomitable', 'the Unconquered', 'the Eternal', 'the Primordial', 'the Unfathomable');
            }
            if (cr >= 20) {
                pool.push('the Almighty', 'the Omniscient', 'the Infinite', 'the Absolute', 'the Supreme', 'the All-Seeing', 'Shaper of Realms', 'Creator of Worlds', 'the First and Last');
            }
        }
        // ─── Layer 3: Alignment ───
        if (isGood) {
            pool.push('the Radiant', 'the Merciful', 'the Benevolent', 'the Compassionate', 'the Gracious', 'the Hopebringer', 'the Dawnbringer', 'the Lightbearer', 'the Protector', 'the Noble', 'the Guardian', 'the Just', 'Shield of the Faithful', 'the Peacemaker', 'the Nurturing');
        }
        if (isEvil) {
            pool.push('the Malevolent', 'the Blighted', 'the Withering', 'the Pestilent', 'the Insidious', 'the Profane', 'the Nightbringer', 'the Forsaken', 'the Devourer', 'the Ravenous', 'the Cruel', 'the Merciless', 'the Pitiless', 'Bringer of Ruin', 'the Defiler');
        }
        if (isLawful) {
            pool.push('Keeper of Oaths', 'the Righteous', 'the Iron-Willed', 'the Arbiter', 'the Ordered', 'Master of Laws', 'the Immutable');
        }
        if (isChaotic) {
            pool.push('the Untamed', 'the Mercurial', 'the Wild', 'Breaker of Chains', 'the Unpredictable', 'the Tempestuous', 'the Unbound', 'the Ever-Changing');
        }
        // Combined alignment titles
        if (isGood && isLawful) {
            pool.push('Lord of Light', 'the Redeemer', 'the Hallowed Sovereign', 'the Paladin Eternal', 'the Shining Judge');
        }
        if (isGood && isChaotic) {
            pool.push('the Liberator', 'the Windborne', 'the Joyous', 'the Wandering Light', 'the Free Spirit', 'the Stormblessed');
        }
        if (isEvil && isLawful) {
            pool.push('the Tyrant', 'Lord of Chains', 'the Iron Sovereign', 'Master of Contracts', 'the Subjugator', 'the Dread Sovereign', 'Keeper of Souls', 'the Dark Arbiter');
        }
        if (isEvil && isChaotic) {
            pool.push('the Destroyer', 'the Ruinous', 'Lord of Carnage', 'the Worldbreaker', 'the Corruptor', 'the Insatiable', 'the Abyssal Lord', 'the Mad God');
        }
        // ─── Layer 4: Element ───
        switch (element) {
            case 'Fire':
                pool.push('the Blazing', 'the Emberlord', 'the Ashen', 'Lord of Cinders', 'the Scorching', 'the Flamecrown', 'the Inferno', 'the Molten', 'Keeper of the Eternal Flame', 'the Volcanic');
                break;
            case 'Water':
                pool.push('the Tidecaller', 'Lord of the Deep', 'the Drowned', 'the Abyssal', 'the Wavecrown', 'the Unfathomed', 'the Tempest', 'Keeper of the Depths', 'the Maelstrom', 'the Torrential');
                break;
            case 'Air':
                pool.push('the Stormborn', 'Lord of Winds', 'the Thunderlord', 'the Cloudwalker', 'the Galeforce', 'the Skyborn', 'the Zephyr', 'Keeper of Storms', 'the Breathless', 'the Howling');
                break;
            case 'Earth':
                pool.push('the Stonelord', 'the Unyielding Mountain', 'the Crystalborn', 'Lord of the Deep Earth', 'the Ironbound', 'the Quaking', 'the Petrified', 'Keeper of the Roots', 'the Monolith', 'the Earthshaker');
                break;
            case 'Positive Energy':
                pool.push('the Resplendent', 'Lord of Radiance', 'the Lifespring', 'the Luminous', 'the Everbloom', 'the Sunforged', 'Keeper of Souls', 'the Incandescent', 'the Vital', 'the Dawning');
                break;
            case 'Negative Energy':
                pool.push('the Deathless', 'Lord of the Void', 'the Entombed', 'the Hollow', 'the Soulreaper', 'the Withered', 'Keeper of the Dead', 'the Necrotic', 'the Hungering Void', 'the Pale');
                break;
        }
        // Dragon color-specific element titles
        if (dragonColor) {
            const dragonColorTitles = {
        ...
        switch (creatureType) {
            case 'dragon':
                pool.push('the Wyrm', 'the Scaled Terror', 'the Winged Shadow', 'the Hoardkeeper', 'Bane of Knights', 'the Dragonlord');
                break;
            case 'celestial':
                pool.push('the Hallowed', 'the Anointed', 'Herald of Dawn', 'Sword of Justice', 'the Eternal Sentinel', 'Voice of the Divine');
                break;
            case 'fiend':
                pool.push('the Accursed', 'the Damned', 'the Infernal', 'the Tormentor', 'the Corruptor', 'Lord of Perdition');
                break;
            case 'undead':
                pool.push('the Deathless', 'the Entombed', 'the Lichborn', 'the Bone Sovereign', 'the Gravecaller', 'the Pale Rider');
                break;
            case 'fey':
                pool.push('of the Glade', 'of the Moonpool', 'Thornweaver', 'Dewdancer', 'Mistwalker', 'of the Silver Brook', 'Songweaver', 'Dreamtender', 'of the Old Wood', 'Starbloom', 'Willowsong', 'of the Twilight Court', 'Leafwhisper');
                break;
            case 'elemental':
                pool.push('the Primordial', 'the Unbound', 'the Elemental Lord', 'the Living Storm', 'the Primal Force');
                break;
            case 'beast':
            case 'monstrosity':
                pool.push('the Apex', 'the Primal', 'the Feral', 'the Untamed', 'the Alpha', 'the Broodmother');
                break;
            case 'giant':
                pool.push('the Colossus', 'the Mountainborn', 'the Thunderstepper', 'the Earthshaker', 'the Towering');
                break;
            case 'plant':
                pool.push('the Rootmother', 'the Evergreen', 'the Verdant', 'the Overgrown', 'the Blossoming');
                break;
            case 'humanoid':
                pool.push('the Cunning', 'the Ruthless', 'the Warlord', 'the Strategist', 'the Commander');
                break;
        }
        // ─── Layer 6: Domain ───
        if (domain) {
            const domainEntry = Object.values(deityDomains).find(d => d.name === domain);
            if (domainEntry === null || domainEntry === void 0 ? void 0 : domainEntry.titles) {
                pool.push(...domainEntry.titles);
            }
        }
        // ─── Layer 7: Deity tier ───
        if (tier === 'greaterDeity') {
            pool.push('the Almighty', 'the Omniscient', 'the Infinite', 'the Absolute', 'Creator of Worlds', 'the First and Last', 'the Undying Flame', 'the Supreme', 'the All-Seeing', 'Shaper of Realms');
        }
        else if (tier === 'lesserDeity') {
            pool.push('the Exalted', 'the Anointed', 'the Chosen', 'the Crowned', 'the Sovereign', 'the Enthroned', 'the Hallowed');
        }
        else if (tier === 'demigod') {
            pool.push('the Risen', 'the Reborn', 'the Twice-Born', 'the Ascended', 'the Blessed', 'the Touched', 'the Awakened One');
        }
        else if (tier === 'avatar') {
            pool.push('the Manifest', 'the Incarnate', 'the Emissary', 'the Herald', 'the Vessel', 'the Chosen Form');
        }
        return randFromArray(pool);
    }
     ...
     */
    function generatePlanarProperName() {
     ...
     */
    function generatePlanarTitle(nodeType, alignment, element) {
    ...
    function materialPlaneNameGenerator(_node) {
     ...
     */
    function planetNameGenerator(_node) {
        if (Math.random() < 0.3) {
            // Descriptive phrase
            const adjectives = [
    ...
    function forestNameGenerator(node) {
        var _a, _b;
        const temperature = (_b = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.temperature) !== null && _b !== void 0 ? _b : '';
        ...
        let pool = [...prefixes];
        if (temperature === 'Cold')
            pool = pool.concat(coldPrefixes);
        if (temperature === 'Warm')
            pool = pool.concat(warmPrefixes);
        return randFromArray(pool) + randFromArray(suffixes);
    }
    function mountainNameGenerator(_node) {
    ...
    function mountainRangeNameGenerator(_node) {
    ...
    function plainsNameGenerator(_node) {
    ...
    function caveNameGenerator(_node) {
        const styles = [
            // "The X Caverns"
    ...
    function forgottenBiomeNameGenerator(node) {
        ...
        const places = (_a = placeNames[node.type]) !== null && _a !== void 0 ? _a : ['Land'];
        return "The " + adjective + ' ' + randFromArray(places);
    }
    function continentNameGenerator(_node) {
        if (queuedName) {
     ...
     */
    function dragonNameGenerator(node) {
        var _a, _b, _c;
        ...
        const cr = (_b = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.challengeRating) !== null && _b !== void 0 ? _b : 1;
        ...
        if (Math.random() < titleChance) {
            ...
            const title = generateTitle({
                alignment: (_c = node.attributes) === null || _c === void 0 ? void 0 : _c.alignment,
                cr,
                creatureType: 'dragon',
                dragonColor
            });
     ...
     */
    function extraplanarNameGenerator(node) {
        var _a, _b, _c, _d;
        ...
        const cr = (_b = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.challengeRating) !== null && _b !== void 0 ? _b : 1;
        const alignment = (_d = (_c = node.attributes) === null || _c === void 0 ? void 0 : _c.alignment) !== null && _d !== void 0 ? _d : '';
     ...
     */
    function feyNameGenerator(node) {
        var _a, _b;
     ...
     */
    function deityNameGenerator(node) {
        var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k;
        const cr = (_b = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.challengeRating) !== null && _b !== void 0 ? _b : 20;
        const alignment = (_d = (_c = node.attributes) === null || _c === void 0 ? void 0 : _c.alignment) !== null && _d !== void 0 ? _d : '';
        const element = (_f = (_e = node.attributes) === null || _e === void 0 ? void 0 : _e.element) !== null && _f !== void 0 ? _f : '';
        ...
        const tier = isGreater ? 'greaterDeity' : isLesser ? 'lesserDeity' : 'demigod';
        const domainName = (_h = (_g = node.attributes) === null || _g === void 0 ? void 0 : _g.domain) !== null && _h !== void 0 ? _h : '';
        ...
        const baseCreature = (_k = (_j = node.attributes) === null || _j === void 0 ? void 0 : _j.creature) !== null && _k !== void 0 ? _k : '';
     ...
     */
    function generateCreatureDerivedName(node, baseCreature) {
     ...
     */
    function avatarNameGenerator(node) {
        var _a, _b;
        const deityName = (_b = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.deityName) !== null && _b !== void 0 ? _b : '';
        if (deityName) {
     ...
     */
    function knighthoodOrderNameGenerator(node) {
        var _a, _b;
        const worship = (_b = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a.worship) !== null && _b !== void 0 ? _b : '';
     ...
     */
    function tavernNameGenerator(_node) {
        const adjectives = [
     ...
     */
    function shopNameGenerator(node) {
        const adjectives = [
     ...
     */
    function feyVillageNameGenerator(node) {
        var _a;
        const parentName = (_a = node.parent) === null || _a === void 0 ? void 0 : _a.name;
     ...
     */
    function archfeyNameGenerator(node) {
        return feyNameGenerator(node);
    }

    // Forward reference — will be populated by the main objectTypes module after merge
    let objectTypesRef$2 = {};
    function setObjectTypesRef$2(ref) {
        objectTypesRef$2 = ref;
    }
    const planarTypes = {
        ...
        avatar: {
            ...
            customSetup: (node) => avatarSetup(node, getRegisteredNodes('greaterDeity', 'lesserDeity', 'demigod'), collectAncestorTags(node, objectTypesRef$2)),
        },
    };
    ...
    // Forward reference — will be populated by the main objectTypes module after merge
    let objectTypesRef$1 = {};
    function setObjectTypesRef$1(ref) {
        objectTypesRef$1 = ref;
    }
    const settlementObjTypes = {
        ...
        metropolis: {
            ...
            customSetup: (node) => settlementSetup(node, objectTypesRef$1),
        ...
        temple: {
            typeName: "Temple",
            nameGenerator: (node) => { var _a; return 'Temple of ' + (((_a = node.attributes) === null || _a === void 0 ? void 0 : _a.worship) || 'the Divine'); },
            attributes: { worship: '' },
    ...
    // Forward reference — will be populated by the main objectTypes module after merge
    let objectTypesRef = {};
    function setObjectTypesRef(ref) {
        objectTypesRef = ref;
    }
    const creatureTypes = {
    ...
    const objectTypes = Object.assign(Object.assign(Object.assign(Object.assign({}, planarTypes), geographyTypes), settlementObjTypes), creatureTypes);
    // Wire up forward references so partial files can access the merged objectTypes
    setObjectTypesRef$2(objectTypes);
    setObjectTypesRef$1(objectTypes);
    setObjectTypesRef(objectTypes);
    setObjectTypesRef$3(objectTypes);
    ...
    function replaceTokensInString(targetString, statBlock, trait) {
        ...
        while (outputString.includes('{{')) {
            ...
            if (statBlock[token] !== undefined) {
                ...
                const statNames = { int: 'Intelligence', wis: 'Wisdom', cha: 'Charisma' };
                ...
                if (tokenArray[0] === 'DC') {
                    ...
                    if (trait[tokenArray[1]] !== undefined) {
                        ...
                        let result = '';
                        if (spellsByUses[0])
                            result += 'At will: ' + formatNames(spellsByUses[0]);
                        ...
                        const parts = [];
                        if (spellsByLevel[0])
                            parts.push(`Cantrips (at will): ${formatNames(spellsByLevel[0])}`);
    ...
    function renderStatblock(sourceStats, target) {
        ...
        for (const traitName in stats.traits) {
            ...
            if (t.spellList) {
                for (const spell in t.spellList)
        ...
        if (stats.bonusArmor) {
            ...
            if (!armorDescription)
                armorDescription = stats.armorDescription ?? 'Bonus Armor';
        }
        ...
        for (const traitName in stats.traits) {
            const t = stats.traits[traitName];
            if (t.hitPointsPerHitDie)
                bonusHP += t.hitPointsPerHitDie * stats.hitDice;
        }
        if (stats.bonusHP)
        ...
        const speedParts = [];
        if (stats.speed)
            speedParts.push(`${stats.speed} ft.`);
        if (stats.swim)
            speedParts.push(`Swim ${stats.swim} ft.`);
        if (stats.climb)
            speedParts.push(`Climb ${stats.climb} ft.`);
        if (stats.burrow)
            speedParts.push(`Burrow ${stats.burrow} ft.`);
        if (stats.fly)
            speedParts.push(`Fly ${stats.fly} ft.`);
        ...
        if (stats.saves || stats.saveBonus) {
            ...
            for (const ability in abilities) {
                let saveModifier = 0;
                if (stats.saves?.includes(ability))
                    saveModifier += stats.proficiency;
                if (stats.saveBonus)
                    saveModifier += stats.saveBonus;
                if (saveModifier > 0) {
        ...
        stats.passivePerception = 10 + stats.abilityModifiers.wis
        ...
        target.appendChild(mkProp('Challenge', stats.cr
            ? `${stringForCR(stats.cr)} (${averageStats[Number(stats.cr)].xp.toLocaleString()} XP)`
        ...
        if (stats.traits && Object.keys(stats.traits).length) {
            ...
            for (const traitName in stats.traits) {
                ...
                if (t.recharge === 'short')
                    resolvedName += ' (Recharges after a Short or Long Rest)';
                else if (t.recharge === 'long')
                    resolvedName += ' (Recharges after a Long Rest)';
                const p = mkEl('p');
        ...
        if (hasAttacks) {
            ...
            if (stats.attacks) {
                for (const attackKey in stats.attacks) {
                    ...
                    if (atk.bonusAttack)
                        atk.attackBonus += atk.bonusAttack;
                    if (atk.bonusDamage)
                        atk.damageBonus += atk.bonusDamage;
                    ...
                    if (maxDmg > 1) {
                        ...
                        if (atk.damageBonus)
                            attackString += (atk.damageBonus >= 0 ? ' + ' : ' - ') + Math.abs(atk.damageBonus);
                        attackString += ')';
                    }
            ...
            if (stats.actions) {
                for (const actionName in stats.actions) {
                    ...
                    if (action.recharge === 'short')
                        actionDisplayName += ' (Recharges after a Short or Long Rest)';
                    else if (action.recharge === 'long')
                        actionDisplayName += ' (Recharges after a Long Rest)';
                    const p = mkEl('p');
        ...
        if (stats.bonusActions && Object.keys(stats.bonusActions).length) {
            ...
            for (const bonusActionName in stats.bonusActions) {
                ...
                if (ba.recharge === 'short')
                    baDisplayName += ' (Recharges after a Short or Long Rest)';
                else if (ba.recharge === 'long')
                    baDisplayName += ' (Recharges after a Long Rest)';
                const p = mkEl('p');
        ...
        if (stats.legendaryActions && Object.keys(stats.legendaryActions).length) {
            ...
            for (const actionName in stats.legendaryActions) {
                ...
                if (la.recharge === 'short')
                    laDisplayName += ' (Recharges after a Short or Long Rest)';
                else if (la.recharge === 'long')
                    laDisplayName += ' (Recharges after a Long Rest)';
                const p = mkEl('p');
    ...
    const getLineages = (race) => race === null || race === void 0 ? void 0 : race.lineages;
    let rootNode;
    let selectedNode;
    ...
    $(function () {
        ...
        $('body').on('click', '.node-label', function (e) {
            e.stopPropagation();
        ...
        $('body').on('click', '.node-toggle', function (e) {
            e.stopPropagation();
        ...
        $('body').on('click', '.button-generate-children', function (e) {
            e.stopPropagation();
            const $node = $(this).closest('.node');
        ...
        $('body').on('click', '.button-view-statblock', function (e) {
            ...
            const creatureID = template.dynamicCreature ? (_a = selectedNode.attributes) === null || _a === void 0 ? void 0 : _a.creature : template.creature;
            const creatureVariant = template.dynamicCreature ? (_b = selectedNode.attributes) === null || _b === void 0 ? void 0 : _b.variant : template.variant;
            const creatureLegendary = template.dynamicCreature ? (_c = selectedNode.attributes) === null || _c === void 0 ? void 0 : _c.legendary : template.legendary;
            if (!creatureID)
                return;
            const cr = String((_e = (_d = selectedNode.attributes) === null || _d === void 0 ? void 0 : _d.challengeRating) !== null && _e !== void 0 ? _e : 1);
            try {
                const scaleOptions = {
                ...
                if ((_f = selectedNode.attributes) === null || _f === void 0 ? void 0 : _f.race) {
                    const raceIndex = races.findIndex(r => r.name === selectedNode.attributes.race);
                    if (raceIndex > 0) {
                        scaleOptions.race = raceIndex;
                        // Resolve lineage name to index
                        const lineageName = selectedNode.attributes.lineage;
                        if (lineageName && getLineages(races[raceIndex])) {
                            const lineageIndex = getLineages(races[raceIndex]).findIndex(l => l.name === lineageName);
                            if (lineageIndex >= 0)
                                scaleOptions.lineage = lineageIndex;
                        }
                    }
                }
                ...
                const genderStr = (_g = selectedNode.attributes) === null || _g === void 0 ? void 0 : _g.gender;
                if (genderStr) {
                    statblock.gender = genderStr === 'Male' ? 1 : genderStr === 'Female' ? 2 : 3;
                }
                // Apply alignment
                if ((_h = selectedNode.attributes) === null || _h === void 0 ? void 0 : _h.alignment) {
                    statblock.alignment = selectedNode.attributes.alignment;
                }
                // Apply extra resistances from node attributes
                if ((_j = selectedNode.attributes) === null || _j === void 0 ? void 0 : _j.extraResistances) {
                    const extras = selectedNode.attributes.extraResistances.split(',').map((s) => s.trim()).filter(Boolean);
                    if (extras.length) {
        ...
        $('#statblock-modal').on('click', function (e) {
            if (e.target === this) {
                $(this).removeClass('is-open');
            }
        });
        // Add child button — generates a single node of the selected type and appends it
        $('#info-panel').on('click', '.button-add-child', function (e) {
            e.preventDefault();
            const childType = $(this).siblings('.add-child-select').val();
            if (!childType || !objectTypes[childType])
                return;
        ...
        $('#info-panel').on('input change', 'input,select,textarea', function () {
            ...
            if (!attribute || $(this).hasClass('add-child-select'))
                return;
            ...
            if (attribute === 'name') {
                const species = ((_a = selectedNode.attributes) === null || _a === void 0 ? void 0 : _a.species) ? selectedNode.attributes.species + ' ' : '';
                ...
                if (attribute === 'species') {
                    const species = ((_b = selectedNode.attributes) === null || _b === void 0 ? void 0 : _b.species) ? selectedNode.attributes.species + ' ' : '';
            ...
            if (attribute === 'creature' && objectTypes[selectedNode.type].dynamicCreature) {
                ...
                const creatureEntry = monsterList[newCreatureID];
                const variants = (creatureEntry === null || creatureEntry === void 0 ? void 0 : creatureEntry.variants) ? Object.keys(creatureEntry.variants) : [];
                ...
                if (variants.length > 0) {
                    ...
                    for (const v of variants) {
                        const variantName = creatureEntry.variants[v].name;
            ...
            if (attribute === 'race' && ((_c = selectedNode.attributes) === null || _c === void 0 ? void 0 : _c.lineage) !== undefined) {
                const newRaceKey = $(this)[0].value;
                const raceEntry = races.find(r => r.name === newRaceKey);
                const lineages = (_d = getLineages(raceEntry)) !== null && _d !== void 0 ? _d : [];
        ...
        $('#button-import-selected').on('click', function () {
            ...
            if (data && data !== "") {
                const newChild = JSON.parse(data);
        ...
        $('#button-delete-selected').on('click', function () {
            ...
            if (!confirm('Delete "' + (selectedNode.name || objectTypes[selectedNode.type].typeName) + '" and all its children?'))
                return;
    ...
    function showInfoForNode(node) {
        ...
        if (node.attributes) {
            for (const attribute in node.attributes) {
                ...
                let $input;
                const templateAttribute = attributeEditors[attribute] || ((_a = template.attributes) === null || _a === void 0 ? void 0 : _a[attribute]);
                if (attribute === 'creature' && template.dynamicCreature) {
                    ...
                    const creatureEntry = monsterList[selectedCreature];
                    const variants = (creatureEntry === null || creatureEntry === void 0 ? void 0 : creatureEntry.variants) ? Object.keys(creatureEntry.variants) : [];
                    if (variants.length > 0) {
                        const $variantLabel = $('<label for="variant" class="variant-label">' + (labels['variant'] || 'Variant') + ': </label>');
                ...
                if (Array.isArray(templateAttribute)) {
                    ...
                    const allKnown = [
                        ...deities.map(d => { var _a; return (_a = d.name) !== null && _a !== void 0 ? _a : d.type; }),
                        ...domainNames,
                        ...dedications
                    ];
                    ...
                    if (deities.length > 0) {
                        ...
                        for (const deity of deities) {
                            const name = (_b = deity.name) !== null && _b !== void 0 ? _b : deity.type;
                            const $opt = $('<option></option>').attr('value', name).html(name);
                            if (name === currentWorship)
                    ...
                    if (isCustom)
                        $customOpt.attr('selected', 'selected');
                    ...
                    const demographics = node.attributes[attribute];
        ...
        if (((_c = node.attributes) === null || _c === void 0 ? void 0 : _c.lineage) !== undefined && ((_d = node.attributes) === null || _d === void 0 ? void 0 : _d.race)) {
            const raceEntry = races.find(r => r.name === node.attributes.race);
            const raceLineages = raceEntry ? getLineages(raceEntry) : undefined;
            if (raceLineages === null || raceLineages === void 0 ? void 0 : raceLineages.length) {
                const $lineageLabel = $('<label for="lineage">' + (labels['lineage'] || 'Lineage') + ': </label>');
        ...
        const creatureID = template.dynamicCreature ? (_e = node.attributes) === null || _e === void 0 ? void 0 : _e.creature : template.creature;
        const creatureVariant = template.dynamicCreature ? (_f = node.attributes) === null || _f === void 0 ? void 0 : _f.variant : template.variant;
        const creatureLegendary = template.dynamicCreature ? (_g = node.attributes) === null || _g === void 0 ? void 0 : _g.legendary : template.legendary;
        if (creatureID) {
            ...
            if ((_h = node.attributes) === null || _h === void 0 ? void 0 : _h.challengeRating) {
            ...
            const genderStr = (_j = node.attributes) === null || _j === void 0 ? void 0 : _j.gender;
            if (genderStr) {
                const genderNum = genderStr === 'Male' ? 1 : genderStr === 'Female' ? 2 : 3;
                creatureLink += '&gender=' + genderNum;
            }
            // Pass alignment
            if ((_k = node.attributes) === null || _k === void 0 ? void 0 : _k.alignment) {
            ...
            if ((_l = node.attributes) === null || _l === void 0 ? void 0 : _l.race) {
                const raceIndex = races.findIndex(r => r.name === node.attributes.race);
                if (raceIndex > 0) {
                    ...
                    if (lineageName && getLineages(races[raceIndex])) {
                        const lineageIndex = getLineages(races[raceIndex]).findIndex(l => l.name === lineageName);
                        if (lineageIndex >= 0)
                            creatureLink += '&lineage-select=' + lineageIndex;
                    }
                }
            }
            // Pass extra resistances — triggers advanced options on the monster scaler
            if ((_m = node.attributes) === null || _m === void 0 ? void 0 : _m.extraResistances) {
        ...
        const templateChildren = (_o = objectTypes[node.type]) === null || _o === void 0 ? void 0 : _o.children;
        if (templateChildren) {
            ...
            const sortedValid = [...validTypes].sort((a, b) => { var _a, _b; return (((_a = objectTypes[a]) === null || _a === void 0 ? void 0 : _a.typeName) || a).localeCompare(((_b = objectTypes[b]) === null || _b === void 0 ? void 0 : _b.typeName) || b); });
            for (const t of sortedValid) {
                $('<option></option>').attr('value', t).html(((_p = objectTypes[t]) === null || _p === void 0 ? void 0 : _p.typeName) || capitalize(t)).appendTo($validSelect);
            }
    ...
    function generateChildrenForNode(node) {
        var _a;
        if (!node.children) {
            node.children = [];
        }
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            let childrenAdded = 0;
            const childTemplates = (_a = objectTypes[node.type]) === null || _a === void 0 ? void 0 : _a.children;
            if (!childTemplates)
                break;
            for (const index in childTemplates) {
                ...
                if (childTemplate.conditions && childTemplate.conditions.length > 0) {
                    valid = childTemplate.conditions.some(condition => {
                        var _a, _b;
                        const nodeValue = (_a = node.attributes) === null || _a === void 0 ? void 0 : _a[condition.attribute];
                        const shouldMatch = (_b = condition.match) !== null && _b !== void 0 ? _b : true;
    ...
    function domObjectForNode(node) {
        var _a, _b;
        const template = objectTypes[node.type];
        let labelText = (_a = template === null || template === void 0 ? void 0 : template.typeName) !== null && _a !== void 0 ? _a : node.type;
        // Prepend species name if present (e.g. "Torcan Wolf Pack")
        if ((_b = node.attributes) === null || _b === void 0 ? void 0 : _b.species) {
            labelText = node.attributes.species + ' ' + labelText;
        }
        if (node.name && node.name.length) {
            labelText += ' (' + node.name + ')';
        }
        const hasChildren = !!(template === null || template === void 0 ? void 0 : template.children);
    ...
    function initSortable(el) {
        Sortable.create(el, {
            ...
            onEnd(evt) {
                ...
                const movedNode = $(movedEl).data('node');
                if (!movedNode)
                    return;
                const oldParentEl = evt.from;
                const newParentEl = evt.to;
                const oldParentNode = $(oldParentEl).closest('.node').data('node');
                const newParentNode = $(newParentEl).closest('.node').data('node');
                ...
                const newIndex = (_a = evt.newIndex) !== null && _a !== void 0 ? _a : newParentNode.children.length;
    ...
    function saveWorld() {
        ...
        if (currentSaveName.length) {
            ...
            let valid;
            do {
                saveName = (_a = prompt(promptMessage)) !== null && _a !== void 0 ? _a : '';
    ...
    function stringifyNodes(node, excludeGmOnly = false) {
        return JSON.stringify(node, (key, value) => {
            if (key === 'domElement' || key === 'parent')
                return undefined;
            ...
            if (excludeGmOnly && key === 'children' && Array.isArray(value)) {
                return value.filter((child) => !child.gmOnly);
            }
            return value;
        });
    }
    ...
    function createWorldFromJSON(worldJSON) {
        ...
        for (const type in objectTypes) {
            if (objectTypes[type].registered)
