(function () {
    ...
    const challengeRatingList = Object.keys(averageStats).map(Number);
    ...
    function dragonNameGenerator(node) {
        ...
        if (Math.random() < titleChance) {
            ...
            const elementPool = (_e = elementTitles[node.type]) !== null && _e !== void 0 ? _e : [];
    ...
    function getTitlePool(node, cr) {
        ...
        if (isGood) {
            if (cr < 5)
                return ['Gentle', 'Humble', 'Quiet', 'Watchful', 'Patient', 'Faithful'];
            if (cr < 12)
                return ['Radiant', 'Merciful', 'Resolute', 'Shining', 'Valiant', 'Blessed', 'Hallowed', 'Anointed'];
        ...
        if (isChaotic && isEvil) {
            if (cr < 5)
                return ['Wretched', 'Crawling', 'Pitiful', 'Sniveling', 'Craven', 'Lurking'];
            if (cr < 12)
                return ['Ravenous', 'Bloodthirsty', 'Savage', 'Vile', 'Corruptor', 'Defiler', 'Flayer', 'Tormentor'];
        ...
        if (isEvil) {
            if (cr < 5)
                return ['Lowly', 'Obedient', 'Dutiful', 'Cringing', 'Servile', 'Minor'];
            if (cr < 12)
                return ['Cunning', 'Ruthless', 'Calculating', 'Merciless', 'Deceiver', 'Tempter', 'Schemer', 'Enforcer'];
        ...
        if (cr < 5)
            return ['Wandering', 'Silent', 'Unseen', 'Drifting'];
        if (cr < 12)
            return ['Enigmatic', 'Inscrutable', 'Formidable', 'Relentless'];
     ...
     */
    function buildDeityTitlePool(isGood, isEvil, isLawful, isChaotic, isGreater, isLesser, element, domain) {
        ...
        // --- Good / Evil axis ---
        if (isGood) {
            pool.push('the Radiant', 'the Merciful', 'the Benevolent', 'the Compassionate', 'the Lifegiver', 'the Healer', 'the Hopebringer', 'the Gracious', 'Shield of the Faithful', 'the Peacemaker', 'the Nurturing', 'the Dawnbringer', 'the Lightbearer', 'the Serene');
        }
        if (isEvil) {
            pool.push('the Malevolent', 'the Blighted', 'the Withering', 'the Pestilent', 'the Insidious', 'the Profane', 'the Nightbringer', 'the Forsaken', 'Lord of Decay', 'the Corrosive', 'the Devourer', 'the Ravenous', 'Bringer of Ruin', 'the Defiler');
        }
        // --- Lawful / Chaotic axis ---
        if (isLawful) {
            pool.push('the Just', 'Keeper of Oaths', 'the Righteous', 'the Iron-Willed', 'the Arbiter', 'the Unyielding', 'the Ordered', 'Master of Laws', 'the Immutable');
        }
        if (isChaotic) {
            pool.push('the Untamed', 'the Mercurial', 'the Wild', 'Breaker of Chains', 'the Unpredictable', 'the Tempestuous', 'the Free', 'the Unbound', 'the Ever-Changing');
        }
    ...
    function showInfoForNode(node) {
        ...
        const creatureID = template.dynamicCreature ? (_b = node.attributes) === null || _b === void 0 ? void 0 : _b.creature : template.creature;
        const creatureVariant = template.dynamicCreature ? (_c = node.attributes) === null || _c === void 0 ? void 0 : _c.variant : template.variant;
        const creatureLegendary = template.dynamicCreature ? (_d = node.attributes) === null || _d === void 0 ? void 0 : _d.legendary : template.legendary;
        if (creatureID) {
            ...
            if ((_e = node.attributes) === null || _e === void 0 ? void 0 : _e.challengeRating) {
