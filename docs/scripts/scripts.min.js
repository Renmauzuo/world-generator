...
var rootNode;
var selectedNode; //Start out true because user doesn't need to be prompted to save an unedited random world
...
$(function () {
  ...
  updateLoadList(); //Start by creating a multiverse
  ...
  $('#info-panel').on('input', 'input,select', function () {
    var attribute = $(this).attr('id'); //If its name changed then update the associated DOM element
  ...
  $('#button-import-selected').on('click', function () {
    ...
    if (data && data != "") {
      //createWorldFromJSON(data);
      var newChild = JSON.parse(data); //This might happen if the user pastes a child onto a type that normally doesn't have children
...
function showInfoForNode(node) {
  ...
  if (node.attributes) {
    for (var attribute in node.attributes) {
      ...
      $label.appendTo($info); //If the template attribute is an array create a picklist
      ...
      if (Array.isArray(templateAttribute)) {
        ...
        for (var index in templateAttribute) {
          let $option = $('<option></option>').html(templateAttribute[index]).appendTo($input); //If this is the current value select it
...
function onToggle() {
  var node = $(this).data('node'); //If node is being opened see if children are needed
  ...
  if ($(this).attr('open')) {
    if (node.needsChildren) {
      ...
      for (var index in objectTypes[node.type].children) {
        ...
        var valid = true; //TODO: Remove this once old requirements are updated to the new way
...
function confirmSaved() {
  if (!saved) {
    ...
    if (shouldSave) {
      ...
      if (currentSaveName.length) {} else {
...
var objectTypes = {
  ...
  planarLayer: {
    ...
    categories: ['plane'],
    inheritAttributes: ["alignment", "element"] //TODO: Planar layer geography
  ...
  reef: {
    typeName: 'Reef' //TODO: Reef children

  },
  lagoon: {
    typeName: 'Lagoon' //TODO: Lagoon children

  },
  openOcean: {
    typeName: 'Open Ocean' //TODO: Open ocean children

  },
  abyssalTrench: {
    typeName: 'Abyssal Trench' //TODO: Abyssal Trench children

  },
  beach: {
    typeName: 'Beach' //TODO: Beach children

  },
  lavaLake: {
    typeName: "Lava Lake" //TODO: Lava lake children
  ...
  sea: {
    typeName: 'Sea' //TODO: Sea children

  },
  tundra: {
    typeName: 'Tundra' //TODO Tundra children

  },
  tropicalForest: {
    typeName: "Tropical Forest" //TODO: Forest children

  },
  deciduousForest: {
    typeName: "Deciduous Forest" //TODO: Forest children

  },
  coniferousForest: {
    typeName: 'Coniferous Forest' //TODO: Forest children
  ...
  coastalCliff: {
    typeName: "Coastal Cliff" //TODO; Cliff children
  ...
  mountain: {
    typeName: 'Mountain' //TODO: Mountain children
...
function planarNameGenerator(node) {
  ...
  var alignmentChaotic = node.attributes.alignment.includes("Chaotic"); //TODO: Add name schemes other than "The X of Y"

  var name = "The "; //Start with some generic location types
  ...
  name += randFromArray(placeOptions) + ' of '; //Start with some generic options just in case this plane is none/none
  ...
  name += randFromArray(domainOptions); //TODO: Maybe add a check to avoid names like "The Clouds of Clouds"
